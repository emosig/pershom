...
def Pareto(f_1,f_2,p1,p2,p3):
    ...
    for i in range (0,l,1):
        ...
        if abs(a[0])<=p3 and abs(a[1])<=p3 and abs(det[i])<=p3:       #se ho un punto critico per f_1 la condizione è verificata per qualunque valore del gradiente di f_2
            ppc.append(x[i])
        if abs(b[0])<=p3 and abs(b[1])<=p3 and abs(det[i])<=p3:       #se ho un punto critico per f_2 la condizione è verificata per qualunque valore del gradiente di f_1
            ppc.append(x[i])

        if abs(a[0])<=p3 and abs(a[1])>p3 and abs(det[i])<=p3:     
            if b[1]/a[1]<=p3:
                ppc.append(x[i])
        if abs(a[1])<=p3 and abs(a[0])>p3 and abs(det[i])<=p3:
            if b[0]/a[0]<=p3:
                ppc.append(x[i])
        if abs(b[0])<=p3 and abs(b[1])>p3 and abs(det[i])<=p3:
            if a[1]/b[1]<=p3:
                ppc.append(x[i])
        if abs(b[1])<=p3 and abs(b[0])>p3 and abs(det[i])<=p3:
            if a[0]/b[0]<=p3:
                ppc.append(x[i])
        if abs(b[0])>p3 and abs(b[1])>p3 and abs(det[i])<=p3:
...
def Pareto2(f_1,f_2,p1,p2,p3,metodo):
    ...
    for i in range (0,l,1):
        ...
        if abs(b[0])<=p2 and abs(b[1])<=p2:       #punto critico per f_2
            cr2.append(x[i])
        if abs(a[0])<=p3 and abs(a[1])<=p3:       #se ho un punto critico per f_1 la condizione è verificata per qualunque valore del gradiente di f_2
            ppc.append(x[i])
        if abs(b[0])<=p3 and abs(b[1])<=p3:       #se ho un punto critico per f_2 la condizione è verificata per qualunque valore del gradiente di f_1
            ppc.append(x[i])
        if abs(a[0])<=p3 and abs(a[1])>p3:     
            if abs(b[0])<=p3 and b[1]/a[1]<=p3:
                ppc.append(x[i])
        if abs(a[1])<=p3 and abs(a[0])>p3:
            if abs(b[1])<=p3 and b[0]/a[0]<=p3:
                ppc.append(x[i])
        if abs(b[0])<=p3 and abs(b[1])>p3:
            if abs(a[0])<=p3 and a[1]/b[1]<=p3:
                ppc.append(x[i])
        if abs(b[1])<=p3 and abs(b[0])>p3:
            if abs(a[1])<=p3 and a[0]/b[0]<=p3:
                ppc.append(x[i])
        if abs(b[0])>p3 and abs(b[1])>p3:
            if abs(a[0]/b[0]-a[1]/b[1])<=p3 and (a[0]/b[0]<=p3 or a[1]/b[1]<=p3):
                ppc.append(x[i])
        if abs(a[0])>p3 and abs(a[1])>p3:
            if abs(b[0]/a[0]-b[1]/a[1])<=p3 and (b[0]/a[0]<=p3 or b[1]/a[1]<=p3):
                ppc.append(x[i])
    ppc=np.array(ppc)
    cr1=np.array(cr1)
    cr2=np.array(cr2)
